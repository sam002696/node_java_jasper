...
@RestController
public class LightEngineeringController {

    private static final Logger logger = LoggerFactory.getLogger(LightEngineeringController.class);

    @PostMapping("/light-engineering-report")
    public ResponseEntity<byte[]> generateReport(@RequestBody LightEngineeringDocument document) {
        try {
            ...
            InputStream reportStream = getClass().getResourceAsStream("/reports/LeGeneralReport.jrxml");
